...
<div class="bg-gray-100 min-h-screen p-6">
    ...
    <div class="bg-white p-6 rounded-b-lg shadow border border-gray-200 max-w-4xl mx-auto mt-2">
        ...
        <h2 class="text-2xl font-bold text-gray-800 mb-6 border-b pb-3 tracking-tight">add a new counselor/mentor</h2>

        @if ($errors->any())
            <div class="bg-red-50 border-l-4 border-red-500 text-red-700 p-3 mb-4 rounded-r text-xs">
                <p class="font-bold">Please correct the following input constraints:</p>
        ...
        <form action="{{ route('admin.mentors.store') }}" method="POST" enctype="multipart/form-data" class="space-y-5">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-5">
                <div>
                    ...
                    <input type="text" name="name" value="{{ old('name') }}" class="w-full border border-gray-300 rounded p-2 text-sm focus:ring-2 focus:ring-green-400 focus:outline-none" placeholder="insert the counselor full name" required>
                </div>
                ...
                <div>
                    ...
                    <select name="platform" class="w-full border border-gray-300 rounded p-2 text-sm focus:ring-2 focus:ring-green-400 focus:outline-none bg-white" required>
                        <option value="onsite" {{ old('platform') == 'onsite' ? 'selected' : '' }}>onsite</option>
                        <option value="online" {{ old('platform') == 'online' ? 'selected' : '' }}>online</option>
                        <option value="onsite and online" {{ old('platform') == 'onsite and online' ? 'selected' : '' }}>onsite and online</option>
                    </select>
                </div>
                ...
                <div>
                    ...
                    <select name="working_days" class="w-full border border-gray-300 rounded p-2 text-sm focus:ring-2 focus:ring-green-400 focus:outline-none bg-white" required>
                        <option value="everyday" {{ old('working_days') == 'everyday' ? 'selected' : '' }}>everyday</option>
                        <option value="weekdays only" {{ old('working_days') == 'weekdays only' ? 'selected' : '' }}>weekdays only</option>
                    </select>
                </div>
                ...
                <div>
                    ...
                    <select name="location" class="w-full border border-gray-300 rounded p-2 text-sm focus:ring-2 focus:ring-green-400 focus:outline-none bg-white" required>
                        ...
                        <option value="Purwokerto" {{ old('location') == 'Purwokerto' ? 'selected' : '' }}>Purwokerto</option>
                    </select>
                </div>
                ...
                <div class="md:col-span-2">
                    <div>
                        ...
                        <input type="text" name="time_schedule" value="{{ old('time_schedule', '08:30 - 16:30') }}" class="w-full border border-gray-300 rounded p-2 text-sm focus:ring-2 focus:ring-green-400 focus:outline-none" placeholder="insert his/her time schedule (e.g., 08:30 - 16:30)" required>
                    </div>
                </div>
            </div>
            ...
            <div>
                ...
                <textarea name="specialty" class="w-full border border-gray-300 rounded p-2 h-28 text-sm focus:ring-2 focus:ring-green-400 focus:outline-none" placeholder="Type additional needs or core specializations (e.g. Mental Wellness, Academic Planning, Financial Counseling)..." required>{{ old('specialty') }}</textarea>
            </div>

            <div>
                <label class="block text-xs font-bold text-gray-700 uppercase tracking-wider mb-1">profile upload</label>
                <div class="mt-1 flex items-center border border-gray-300 rounded p-2 bg-gray-50">
                ...
                <p class="text-[10px] text-gray-400 mt-1">Accepted resource types: png, jpg, jpeg. Maximum capacity dimensions: 2MB.</p>
            </div>
